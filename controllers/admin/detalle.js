...
SEARCH_FORM_DETALLE.addEventListener('submit', (event) => {
    ...
    const idPedido = ID_PEDIDO.value;  // Asegurarse de tener el ID del pedido
    fillTableDetalle(idPedido, FORM);
});
...
const fillTableDetalle = async (idPedido, form = null) => {
    ...
    const DATA = await fetchData(DETALLES_PEDIDOS_API, action, form);
    if (DATA.status) {
        DATA.dataset.forEach(row => {
            TABLE_BODY_DETALLE.innerHTML += `
                <tr>
...
const openDeleteDetalle = async (id) => {
    ...
    if (RESPONSE) {
        ...
        if (DATA.status) {
            sweetAlert(1, DATA.message, true);
            fillTableDetalle(ID_PEDIDO.value);
        } else {
            sweetAlert(2, DATA.error, false);
        }
    }
