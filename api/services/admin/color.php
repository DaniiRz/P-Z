    ...
    if (isset($_SESSION['idAdministrador'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'createRow':
                ...
                if (
                    !$Color->setNombreColor($_POST['nombreColor'])
                ) {
                    $result['error'] = $Color->getDataError();
                } elseif ($Color->createRows()) {
                    $result['status'] = 1;
            ...
            case 'readAll':
                if ($result['dataset'] = $Color->readAll()) {
                    ...
                    $result['error'] = 'No existen Colores registradas';
                }
                break;
            ...
            case 'updateRow':
                $_POST = Validator::validateForm($_POST);
                if (
                    !$Color->setIdColor($_POST['idColor']) or
                    !$Color->setNombreColor($_POST['nombreColor'])
                ) {
                    $result['error'] = $Color->getDataError();
                } elseif ($Color->updateRows()) {
                    $result['status'] = 1;
            ...
            case 'deleteRow':
                if (
                    !$Color->setIdColor($_POST['idColor'])
                ) {
                    $result['error'] = $Color->getDataError();
                } elseif ($Color->deleteRows()) {
                    $result['status'] = 1;
